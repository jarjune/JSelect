function JSelect(el, level, params, funcs, customFunc) {
    ...
    // 正在选中项
    this.changeArr = params;

    // 最终选中项
    this.changedArr = params;
    ...
    this.setVal = function() {
        ...
        for(var i = 0; i <= this.tabIndex; i++) {
            if(this.changeArr[i] != undefined) {
                _str += this.changeArr[i].name + ' ';
            }
        }
    ...
    this.initBar = function() {
        for(var i = 0; i <= this.tabIndex; i++) {
            ...
            if(i == 0) {
                ...
                $('.selected-address li').eq(i).data('pcode', this.changeArr[i-1].code || '');
            }
            $('.selected-address li').eq(i).data('code', this.changeArr[i].code || '');
            $('.selected-address li').eq(i).text(this.changeArr[i].name || this.defaultTabText);
        }
    }
    ...
    this.bindEvent = function() {
        ...
        $('.selected-address li').click(function() {
            _this.reset($(this).index())
            _this.reloadContent(_this.tabIndex = $(this).index(), $(this).data('pcode'))
        })
    }
    ...
    this.initContent = function() {
        var init_code = 0;
        if(this.changeArr[this.tabIndex-1] != undefined) {
            init_code = this.changeArr[this.tabIndex-1]['code']
        }
    ...
    this.reloadContent = function(index, pcode) {
        $('.selected-address li').eq(index).addClass('active').siblings().removeClass('active')
